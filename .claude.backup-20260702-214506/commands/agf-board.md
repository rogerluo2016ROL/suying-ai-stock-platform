...
3. 看板含义一句话给用户：三列 = Pending / In Progress / Completed；卡片左边条色 = 角色，`· N 分钟前` = 该卡最后一次状态更新；⛓ = blocked by；成员栏 = 全队名单（◆ lead / ● 工作中→关联卡号，无卡片时内联 spawn prompt 摘要 + hover 看全文 / ○ 空闲；来自 team config，solo session 自动隐藏）；顶部 chips = 阶段门（实现 → Code Review → UAT 部署 → E2E → UAT）。
