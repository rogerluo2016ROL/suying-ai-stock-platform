...
def get_sector_index(db, industry, trade_date, code=None):
    ...
    V5.1-5.2 使用 index_basic→index_daily, 每天仅8个大指数覆盖不足.
    V5.3: 通过 ths_member + ths_daily 获取股票所属概念的实时涨跌幅.
    """
    # 1. THS概念路径 (最完整, ≥1500个概念每日)
    if code:
        row = db.execute(
            "SELECT d.change_pct FROM ths_daily d "
            "JOIN ths_member m ON m.ts_code = d.code "   # m.ts_code = concept code
            "JOIN ths_index i ON m.ts_code = i.ts_code "  # concept name lookup
            "WHERE m.con_code LIKE ? AND d.trade_date = ? "
            "ORDER BY ABS(d.change_pct) DESC LIMIT 1",     # 变动最大的概念
            (f"{code}%", trade_date)
        ).fetchone()
