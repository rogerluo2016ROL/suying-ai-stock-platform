...
- **任何 agent**：撞到「触发硬停」阈值时必须立即停下，不得自作主张续跑

## Cache 利用率（必须 ≥ 60%）

- 高 cache miss 是模板化文档过长 / agent 频繁切换造成的；连续两次会话 cache hit < 50% 应触发优化
- 优化路径：把不常变的内容下沉到 skill / `.claude/standards/`、避免在 CLAUDE.md 反复修改大段
...
不是所有任务都需要 Opus。当 product-lead 判断任务足够明确、不涉及架构权衡时，鼓励 teammate 主动降级到 Sonnet 甚至 Haiku：
...
降级是工具，不是义务——若 Sonnet 多次产出不达预期，立即升回 Opus 不要硬撑。
...
| growth-analyst | sonnet | 实验设计 + 统计判断，需推理深度，不下沉 haiku |
...
[ADR-001](../../docs/adr/001-multi-instance-worker-pool.md) + [`workflow.md` §Multi-instance Worker Pool](workflow.md) 引入同 type 多实例并发；本节定义 pool 模式下的成本约束。
...
PL 在 Step 3 派单时按 PRD 复杂度选档；超档自动告警 retro。

### 触发审查的硬阈值

- **Pool 实际倍数 > 8×** → retro 复盘是否过度并发（在 retro skill §3 token 段记录）
- **单 feature 总 cost > Large 档 1.5×** → tech-lead 评估是否架构问题（如多实例 review 重复读相同 PRD 导致 token 重复）
- **Cache hit ratio < 50%**（pool 模式下，下限比单实例 60% 放宽，因为多实例首次 spawn 必有 cache miss）→ 提示 PL 检查实例是否短时间内 reap 太多
