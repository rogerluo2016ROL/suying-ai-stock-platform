...
import com.ds.cockpit.screen.system.mapper.GacDownloadLogsMapper;
...
@Slf4j
@Service
public class GacDownloadLogsServiceImpl implements IGacDownloadLogsService {

    @Resource
    private GacDownloadLogsMapper gacDownloadLogsMapper;
    ...
    @Override
    public List<GacDownloadLogs> selectListBy(HttpServletRequest request, GacDownloadLogs gacDownloadLogs){
        log.info("开始查询数据下载日志记录数据");
        List<GacDownloadLogs> gacDownloadLogsList = gacDownloadLogsMapper.selectListBy(gacDownloadLogs);
        return gacDownloadLogsList;
    }

    @Override
    public int addEvaluationAnalysis(HttpServletRequest request, GacDownloadLogs gacDownloadLogs) {
        try {
            SSOUserInfoResVo ssoUserInfoResVo = checkArgument(request);
            log.info("成功获取用户信息");
            gacDownloadLogs.setCreateCode(ssoUserInfoResVo.getAccount()+"-"+ssoUserInfoResVo.getUsername());
            gacDownloadLogs.setCreateBy(ssoUserInfoResVo.getNickname());
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
        log.info("开始保存数据下载日志记录数据");
        return gacDownloadLogsMapper.insertGacDownloadLogs(gacDownloadLogs);
    }
