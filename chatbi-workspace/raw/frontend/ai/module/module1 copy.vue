    ...
    <div>
        ...
        <div v-show="dataText.length > 0" class="content" id="yourElementId" ref="container" @scroll="handleScroll">
            ...
            <div v-for="(item, index) in dataText">
                ...
                <el-row v-if="index == dataText.length - 1">
                                ...
                                <el-collapse-item :name="index">
                                    ...
                                    <div style="font-size: 12px;color: #999;">
                                        ...
                                        <el-steps direction="vertical" :active="item.active" finish-status="success"
                                            ...
                                            <el-step title="调度LLM大模型" v-show="item.active >= 1"
                                                :class="item.modelState ? 'bor-cor' : ''">
                                                <template slot="icon">
                                                    <i v-show="!item.modelState && item.reflectOn2 != '当前使用人数较多，请重新加载页面后再次尝试'"
                                                        class="el-icon-loading"></i>
                                                    <i class="el-icon-close"
                                                        v-show="item.modelState && item.reflectOn2 != '当前使用人数较多，请重新加载页面后再次尝试'"></i>
                                                    <i class="el-icon-refresh-right"
                                                        v-show="item.reflectOn2 == '当前使用人数较多，请重新加载页面后再次尝试'"></i>
                                                </template>
                                                <div slot="title" class="title-text">
                                                    调度LLM大模型</div>
                                                <div slot="description" style="color: #585353;">
                                                    <Markdown ref="childRef" :key="index" :content="item.reflectOn2" />
                                                </div>
                                            ...
                                            <el-step title="关键数据检索" v-show="item.active >= 2">
                                                <template slot="icon">
                                                    <i class="el-icon-check" style="color: gold;"></i>
                                                </template>
                                                <div slot="title" class="title-text">
                                                    关键数据检索</div>
                                                <div slot="description" style="color: #585353;">
                                                    <Markdown ref="childRef" :key="item.reflectOn3"
                                                        :content="item.reflectOn3" />
                                                </div>
                                            </el-step>
                                        </el-steps>
                                    </div>
...
export default {
    ...
    methods: {
        ...
        sendApi(id) {//发送功能
            ...
            const intervalStop = setInterval(() => {
                ...
                if (countdown < 0) {
                    ...
                    this.dataText[this.dataText.length - 1].reflectOn2 = '当前使用人数较多，请重新加载页面后再次尝试'
                    this.cancelRequest()
                    this.stopApi()
                    // console.log("倒计时结束");
                }
        ...
        async getText() {//复制文本
            this.textToCopy = this.dataText[this.dataText.length - 1].text
            try {
        ...
        setText() {//编辑文本
            this.input = this.dataText[this.dataText.length - 1].text
        },
        cancelRequest() { //终止请求
            if (this.controller) {
                this.controller.abort();
