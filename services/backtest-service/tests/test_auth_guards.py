...
def test_factors_requires_auth(client):
    resp = client.get("/api/v1/backtest/factors")
    assert resp.status_code == 401


def test_run_rejects_wrong_service_auth(client):
    resp = client.post(
        "/api/v1/backtest/run",
...
def test_factors_accepts_valid_jwt(client):
    resp = client.get(
