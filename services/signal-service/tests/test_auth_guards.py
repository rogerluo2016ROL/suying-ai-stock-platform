...
def test_analyze_requires_auth(client):
    resp = client.get("/api/v1/signal/analyze/600000")
    assert resp.status_code == 401


def test_trigger_sync_rejects_wrong_service_auth(client):
    resp = client.post(
        "/api/v1/signal/trigger-sync?table_key=stocks",
        headers={"X-Service-Auth": "wrong-secret"},
    )
    assert resp.status_code == 401


def test_trigger_sync_rejects_plain_user_role(client):
    resp = client.post(
        "/api/v1/signal/trigger-sync?table_key=stocks",
...
def test_levels_accepts_valid_jwt(client):
    resp = client.get(
