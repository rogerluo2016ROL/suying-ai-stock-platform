...
class BreakerState:
    ...
    probing_count: int = 0  # number of half-open probes attempted
...
async def can_trade(account_id: str) -> tuple[bool, str]:
    ...
    - HALF_OPEN: allowed if probing_count < 1 (one probe only).
    - TRIGGERED: blocked.
    ...
    # Ensure latest state evaluation (cooldown transition)
    await check_daily_loss(account_id)
