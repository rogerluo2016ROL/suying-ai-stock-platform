...
这些数字不是说要盲目采集 224 个接口，而是要求每个接口都进入同一张项目目录，并继续补明确结论：采集、暂缓、无关、权限不足或替代来源。
...
当前 224 个接口已经全部进入项目目录，其中 191 个还没有治理结论。下一步不是全量建表，而是给这些接口打标签：
...
3. 给 191 个待分类 API 打标签，形成正式 Tushare 覆盖矩阵。
