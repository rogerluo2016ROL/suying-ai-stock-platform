# Schema Drift Audit Report — 2026-07-11
**ADR-014 | UAT PG(16432) read-only**

## §1 审计范围

- DB 366张 | init_sql 75张 | 审计 152张 | 排除 23张
...
|5|`block_trade_data`|14|10|low|=|y|
...
|150|`tushare_api_parameter_catalog`|6|0|low|init||
|151|`tushare_api_update_metadata`|7|7|low|=||
|152|`weekly_kline`|13|9|low|=|y|
...
- `daily_basic` (low MON=yes): legacy索引: idx_daily_basic_code_date,idx_daily_basic_date,idx_daily_basic_trade_code,idx_daily_basic_trade_date_code
...
- `dividend_data` (low MON=yes): legacy索引: idx_dividend_data_ex_date_code
...
- `share_float` (low MON=yes): legacy索引: idx_share_float_float_date_code
...
- `stock_news_tushare` (low MON=yes): legacy索引: idx_stock_news_tushare_pub_time
...
- `tushare_api_parameter_catalog` (low MON=no): init独有(DB缺)
...
|daily_basic|idx_daily_basic_trade_date_code|(trade_date,code)|init:n/DB:y|i-miss|
...
|dividend_data|idx_dividend_data_ex_date_code|(ex_date,code)|init:n/DB:y|i-miss|
...
|share_float|idx_share_float_float_date_code|(float_date,code)|init:n/DB:y|i-miss|
...
|stock_news_tushare|idx_stock_news_tushare_pub_time|(pub_time)|init:n/DB:y|i-miss|
