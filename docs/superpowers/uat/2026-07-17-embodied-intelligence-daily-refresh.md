...
> 上线/回滚提醒：当前版本为 Alembic 038。036/037/038 的 `downgrade` 会删除或裁剪刷新运行、证据发布主体、审计快照、送达记录、映射冲突和迁移历史。生产回滚前必须先备份这些表；未备份执行 downgrade 会永久丢失审计历史或发布主体身份，不能作为无损回滚方案。
...
- Alembic `035 -> 036 -> 037 -> 038` 成功；037 增加冲突 proposed-node 列表，038 为证据事件增加可空的可信 `publisher_id` / `canonical_source_id`，禁止用记录 `source_id` 冒充发布主体。
