...
Extend both existing ExtractedFact value objects with metadata: dict[str, Any] | None = None. A pending fact copies only sanitized document metadata; review_normalization is always removed. The copy must preserve explicit route fields such as application_domain, installation_position, revenue_confirmed, legal_status and legal_status_date, but automatic extraction must not invent them from a generic keyword hit. Add a focused test proving those fields survive raw-document to pending-fact persistence while the reserved review key does not.
...
Fact approval also accepts an optional typed EvidenceFactMetadataPatch with only application_domain (dexterous_hand, robot_hand, robot_joint, robot_wrist or automotive), installation_position, revenue_confirmed, profit_confirmed, legal_status and legal_status_date. Only an approved decision may apply this patch. The repository merges it into clean_metadata inside the same guarded transaction, and the response returns the stored metadata. Add tests proving an approved route metadata patch survives the transaction, an unknown field is rejected, and rejected/needs_more_evidence decisions cannot write the patch. This is the manual production path for AF metadata that a collector cannot safely infer.
...
persist_pending_document() must round-trip the sanitized RawDocument metadata into evidence_extracted_facts.metadata. Add a repository test covering application_domain, installation_position and patent legal metadata. Official adapters may populate application_domain or installation_position only from an explicit same-document product-and-scene match; they must never set revenue_confirmed or profit_confirmed automatically. Those confirmation flags require the typed manual review patch from Task 4.
...
derive_route_gate() consumes the Task 2 route predicate contract generically, including any/all fact matching, clause-specific metadata and application domains, route-local fact contracts and fact-level excluded-domain policy. It must not hard-code AF1 or AF6. Add tests proving either an approved active patent or an approved laboratory prototype can satisfy AF1; AF6 requires both an approved order and approved revenue fact; an automotive fact is ignored rather than excluding a company that also has qualifying robot evidence; and pending or unaudited route metadata cannot satisfy any AF stage.
...
7. open one caller-owned transaction for steps 7-9, create a synthetic mapping and pending fact carrying route metadata, approve it with an EvidenceFactMetadataPatch, prove application_domain/installation_position/revenue_confirmed or patent legal metadata round-trips from pending persistence through manual review into derive_route_gate(), pass that connection into review_fact(), read the timezone-aware reviewed_at returned by PostgreSQL, convert it with reviewed_at.astimezone(ZoneInfo('Asia/Shanghai')).date(), and use that as run_batch_score(connection=connection) trade_date; keep the transaction open and assert the reviewed fact is visible only to that current-or-later synthetic score;
