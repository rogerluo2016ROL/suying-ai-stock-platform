...
变更文件夹（`docs/changes/<change>/`）是 AGF 自 v6.9.0 起的**新需求入口**，取代 PRD（OpenSpec 风格，决策见 ADR-012）。它把需求表达成**对活规格的 delta**，交付签字后 merge 进 `docs/specs/`——让「系统当前有哪些行为」永远有 SSOT。
...
简单 change 删掉本文件即可。高风险 change（auth / schema 迁移 / LLM 切换 / cross-cutting）**引用一个新 ADR**（tech-lead 写），不在 design.md 自拍架构。API 契约 SSOT 仍是 OpenAPI（ADR-006）。

### 4. `tasks.md` —— 实现 checklist + AC↔scenario 映射

**关键**：维护 `AC-N ↔ <capability> / Requirement / Scenario` 映射表。AC 仍是 `AC-N`（编号 / 优先级 / progress 5 段格式 / SIT hook 全不变，ADR-012 决策 4），语义来源是 delta 的 scenario——dev 逐 AC 自验、qa 逐 AC 测、PL 逐 AC 签字，照旧。
