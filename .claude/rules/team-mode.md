...
4. Lead 固定为 `product-lead`，且 **lead = 主 session 本身**：由 `claude --agent product-lead` 启动（原 TUI 启动器 `setup/agf-team-start.sh` 已按 ADR-026 D3 退役；普通 session 用 `/agf-team-start` 触发亦可）。PL 走 `--agent` 路径，其 frontmatter（`permissionMode: acceptEdits` / `skills` / `memory`）**全生效**，团队权限基线 = PL 的 `acceptEdits`（**默认不再用 `--dangerously-skip-permissions`**）。**因此不要再把 `product-lead` 作为 teammate spawn**——PL 自身的首个任务（如起 PRD）由本 session 直接执行。前提：`product-lead.md` 的 `tools` 须含 `Agent`（spawn teammate 必需）。
5. 显示模式由 `.claude/settings.json` 的 `teammateMode` 控制（当前 `auto`，2026-07-08 由 `iterm2` 回退见 ADR-022；iTerm2+it2 仍走原生分屏、其他环境优雅 in-process；想强制 fail-loud 的用户在用户级改 `iterm2`），不要在 prompt 里指定 pane 行为。
