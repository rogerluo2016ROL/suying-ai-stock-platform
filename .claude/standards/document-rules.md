...
| 角色 | 文档类型 | 路径规范 | 触发时机 |
|---|---|---|---|
| `product-lead` | 变更文件夹（需求入口）| `docs/changes/[change]/`（proposal / specs delta / design / tasks）| 分配任务前（必须，skill `agf-writing-change`） |
| `product-lead` | 活规格（行为 SSOT）| `docs/specs/[capability]/spec.md` | UAT 签字后 `agf-spec-archive` merge delta（永远当前） |
| `product-lead` | ~~PRD~~（弃用 v6.9.0→删 v7.0.0）| `docs/prd/[feature]-[YYYY-MM-DD].md` | 仅 fallback；新需求走变更文件夹 |
...
| `code-reviewer` | 审查报告 | `docs/reviews/[feature]-[YYYY-MM-DD].md`（单实例）/ `docs/reviews/[feature]-r<N>-[YYYY-MM-DD].md`（pool 实例 N，详 ADR-001）| 每次完成代码审查（必须）；YAML frontmatter 必填 |
...
| 系统当前行为（行为需求）| `docs/specs/[capability]/spec.md`（活规格，永远当前；delta 演进见 ADR-012）| 引用能力路径，不重复行为契约；功能变更走 delta 不手改活规格 |
| 一次变更的需求与验收 | `docs/changes/[change]/`（proposal + specs delta + tasks 的 AC↔scenario 映射；PRD fallback `docs/prd/`）| 任务分配时摘录相关 AC 条目（唯一允许的必要复制） |
