...
- **review 通过 ≥ 2 时**（Step 3.2→3.3 衔接）：触发 qa pool
...
加 `--dry-run` flag 时，**主 Claude 不调 Agent 工具 spawn**，而是输出"将要 spawn 的 teammate 清单 + 预估 token + Pool 触发的 type 与实例数"，然后停。用途：
...
|---|---|---|---|
| product-lead | product-lead | 起 PRD | 单实例（锁定） |
...
Spawn teammates **by subagent type name only** (do NOT use `@` file references — definitions are auto-loaded by Claude Code).

**Teammate 选择规则**：
- 默认 spawn 下表 5 个 teammate
- **如果任务消息含 `[teammates: A,B,C]` 标记行**（脚本入口会带），则**仅** spawn 标记中列出的 teammate；每个的 initial task 由你按 feature 描述与 PRD 上下文合理推断（参考下表的 initial task 模板）

| Teammate | Initial task 模板（默认 / 推断参考） |
|---|---|
| `product-lead` | 调用 `superpowers:brainstorming` 澄清需求，输出 PRD 草稿到 `docs/prd/<feature>-<YYYY-MM-DD>.md`，列出至少 3 个开放问题 |
| `tech-lead` | 检查 `docs/adr/` 是否覆盖本任务所需技术基线；已覆盖则合规检查并回报，未覆盖则起草新 ADR 大纲 |
| `frontend-dev` | 阅读 `src/` / `frontend/`（如存在）后输出页面骨架与状态管理切分计划 |
| `backend-dev` | 阅读 `backend/`（如存在）后输出 API 契约草案（路径、请求/响应 schema） |
| `qa-engineer` | 阅读 PRD 草稿后准备 E2E/UAT 测试策略框架（先不执行；SIT 由 dev 自跑，不在 qa 范围） |
...
1. **必须**是 agent team，不是 subagent。Spawn 完成后**显式确认**："This is an agent team, not subagents."
2. Lead 固定为 `product-lead`。
3. 报告每个 teammate 的 **name + agent ID**：必须用 **markdown 表格**输出，**1 行/teammate**，列 = `name | agent ID | initial task 一句话`。**禁止**为每个 teammate 写一段散文式介绍、禁止复述 initial task 全文、禁止预告"接下来 X 会 Y"。整段 spawn 汇报（确认句 + 表格）合计 ≤ 15 行。
4. Spawn 之后由 `product-lead` 接管协调，按 `.claude/standards/workflow.md` 推进交付链路（dev 自跑 Unit + SIT → code review（含 SIT Audit）→ E2E → UAT）。
5. 多个同类型实例并行时遵守 `.claude/standards/workflow.md` "Parallel Dispatch" 节（文件归属 + 临界区 + 完成报告列全文件）+ §"Multi-instance Worker Pool"（实例命名 `<type>-<N>` / worktree 隔离 / Docker 端口偏移 / 失败处理）。
6. 显示模式由 `.claude/settings.json` 的 `teammateMode` 控制，不要在 spawn prompt 里指定 pane / tmux 行为。

# 任务规模过小怎么办

如果任务明显单角色就能完成（仅文案改动 / 单点 bugfix / 纯查询），不要建 team——直接告诉用户："本任务规模过小，建议改用 `请启动 <角色>` 的单角色 subagent 路径，无需启用 agent team。"

# 另一种路径：Agent View（独立并行执行）

> 适用 Claude Code v2.1.139+。本节不走 `.claude/rules/team-mode.md` 的 Agent Team 协议，因为没有 lead / mailbox / 共享 task list。

## 何时选这条路（而不是 Agent Team）

任务可拆成 **≥2 个互相独立的执行单元**、且满足以下**全部**条件：

- 子任务之间不需要中途讨论 / 互相质疑 / 共享发现（每个 worker 拿到 spec 就能独立完工）
- 已有 PRD 或明确 spec（不需要 product-lead 现场澄清）
- 每个子任务有清晰的交付物边界（独立 PR / 独立 feature 子目录 / 独立报告）
...
- **不能替代 product-lead 协调**：本路径**不做** PRD 演化、不做 UAT 签字、不做跨 worker 冲突仲裁；这些仍需 `/agf-team-start` 走 Agent Team 路径
- **本路径不写 progress/<role>.md**：Self-Reporting Pattern（`.claude/standards/ac-lifecycle.md`）默认绑定 Agent Team；如需保留底稿，在每个 `--bg` 的 prompt 里显式要求 worker 在完成时 append 到 `progress/<role>.md`
- **`permissionMode: auto` / `bypassPermissions` 的 agent 不能直接 `--bg`**：[Claude Code 安全门](https://code.claude.com/docs/en/agent-view#permission-mode-and-settings)规定，未在交互模式接受过该 mode 之前，`--bg` 启动这两类 agent 会被拒绝。哪些 agent 落在这两类，以 `.claude/standards/team-roles.md` "Team Roles" 表的 `Permission` 列为准（不在本文复述名单，避免双写）。规避方法：先 `claude --agent <name>` 在交互模式跑一次接受弹窗，之后即可用 `--bg` 派工
