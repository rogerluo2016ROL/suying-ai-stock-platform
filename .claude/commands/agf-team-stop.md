...
关闭已完成本 feature 工作的执行层 teammate，释放 process / 上下文，但保留 product-lead 与单实例长期角色待命。

**默认关闭类型**（9 个执行层）：`frontend-dev` / `backend-dev` / `ai-agent-dev` / `ml-engineer` / `miniapp-dev` / `code-reviewer` / `miniapp-code-reviewer` / `qa-engineer` / `miniapp-qa-engineer`

**默认保留类型**（5 个单实例长期角色）：`product-lead` / `tech-lead` / `uiux-designer` / `content-writer` / `growth-analyst`

> 范围设计依据：详见 `CLAUDE.md ## Verified Facts` 的 Pool 上限表 —— Pool 上限 = 1 的角色保留，Pool 上限 ≥ 3 的执行层默认关闭。
...
4. **逐个发 shutdown_request**（按 `name` 升序串行，避免响应混乱）：
   ```
   SendMessage({
     to: "<teammate-name>",
...
- 用户表示想关 PL / 长期角色一起 → 不在本命令范围；告诉用户：`"PL 默认保留以接续后续需求；如确需全 team 解散，先手动 SendMessage shutdown_request 给 PL，然后 TeamDelete。"`
