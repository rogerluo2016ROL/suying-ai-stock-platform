...
#       与 agf-board.sh 同数据源）→ 追加一行 pending task 汇总（≤ 3 条摘要）；
...
if [[ "$SOURCE" == "resume" || "$SOURCE" == "compact" ]]; then
  if ls "$PROJECT_DIR"/progress/*.md >/dev/null 2>&1 && \
    ...
    ADDITIONAL='[团队态恢复] progress/ 有进行中的角色底稿。若你是 PL lead，请：① 重读 progress/<role>.md 恢复各 teammate 进度；② 跑 `/agf-board`（或 `bash .claude/scripts/agf-board.sh`）查 task 看板；③ 注意 in-process teammate 不随 /resume 恢复，缺失的需重新 spawn。'
  fi
fi

# 1b) pending task 汇总（仅 resume / compact；best-effort，全程 fail-open）
#     数据源同 .claude/scripts/agf-board.sh：~/.claude/tasks/<list-id>/*.json，
...
if [[ "$SOURCE" == "resume" || "$SOURCE" == "compact" ]] && command -v jq >/dev/null 2>&1; then
  ...
  if [[ -n "$SESSION_ID" && -d "$TASK_DIR" ]] && ls "$TASK_DIR"/*.json >/dev/null 2>&1; then
    ...
    if [[ -n "$PENDING" ]]; then
      ...
      TASK_LINE="[task list] pending task ${PENDING_N} 个：${PENDING_TOP}（详情跑 /agf-board 看板）"
