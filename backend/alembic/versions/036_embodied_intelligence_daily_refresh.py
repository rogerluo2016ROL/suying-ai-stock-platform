...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("source_record_ids", sa.JSON(), nullable=False, server_default=sa.text("'[]'::json")),
        sa.Column("evidence_fingerprints", sa.JSON(), nullable=False, server_default=sa.text("'[]'::json")),
    ...
    op.create_table(
        ...
        sa.Column("code", sa.Text(), nullable=False),
        sa.Column("node_id", sa.Text(), nullable=False),
        sa.Column("from_status", sa.Text(), sa.CheckConstraint("from_status IS NULL OR from_status IN ('candidate','pending_review','verified','weak_evidence','rejected')", name="ck_embodied_mapping_transition_from_status")),
        sa.Column("to_status", sa.Text(), sa.CheckConstraint("to_status IN ('candidate','verified','weak_evidence','rejected')", name="ck_embodied_mapping_transition_to_status"), nullable=False),
        sa.Column("source_record_ids", sa.JSON(), nullable=False, server_default=sa.text("'[]'::json")),
        sa.Column("evidence_fingerprints", sa.JSON(), nullable=False, server_default=sa.text("'[]'::json")),
        sa.Column("source_names", sa.JSON(), nullable=False, server_default=sa.text("'[]'::json")),
        sa.Column("reason", sa.Text(), nullable=False),
        sa.Column("review_status", sa.Text(), sa.CheckConstraint("review_status IN ('pending_review','approved','rejected')", name="ck_embodied_mapping_transition_review_status"), nullable=False, server_default="pending_review"),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
